--  Standalone test suite for Chans_Algorithm (main program).
...
with Ada.Text_IO;
with Chans_Algorithm; use Chans_Algorithm;
...
procedure Tests is
   ...
   function Raised_Invalid_Graham (Pts : Point_Set) return Boolean is
   begin
      declare
         H : constant Point_Array := Graham_Scan_Hull (Pts);
      begin
         pragma Unreferenced (H);
         return False;
      end;
...
begin
   Ada.Text_IO.Put_Line ("Chans_Algorithm tests");
   ...
   Check (Raised_Invalid_Graham (Empty_Set), "Graham empty raises");
   Check (Raised_Invalid_Hull (Too_Many), "Hull oversized raises");
   Check (Raised_Invalid_Andrew (Too_Many), "Andrew oversized raises");
   Check (Raised_Invalid_Graham (Too_Many), "Graham oversized raises");
   ...
   declare
      ...
      H1, H2, HD : Point_Array (1 .. Max_Points);
   ...
   begin
      ...
      declare
         TA : constant Point_Array := Andrew_Monotone_Chain (One);
         TG : constant Point_Array := Graham_Scan_Hull (One);
      begin
         Check (Same_Vertex_Set (H1 (1 .. N1), TA), "Andrew matches single");
         Check (Same_Vertex_Set (H1 (1 .. N1), TG), "Graham matches single");
      end;
      declare
         TA : constant Point_Array := Andrew_Monotone_Chain (Two);
         TG : constant Point_Array := Graham_Scan_Hull (Two);
      begin
         Check (Same_Vertex_Set (H2 (1 .. N2), TA), "Andrew matches two");
         Check (Same_Vertex_Set (H2 (1 .. N2), TG), "Graham matches two");
      end;
      declare
         TA : constant Point_Array := Andrew_Monotone_Chain (Dup);
         TG : constant Point_Array := Graham_Scan_Hull (Dup);
      begin
         Check (Same_Vertex_Set (HD (1 .. ND), TA), "Andrew matches dups");
         Check (Same_Vertex_Set (HD (1 .. ND), TG), "Graham matches dups");
      end;
   end;
   ...
   declare
      ...
      G : constant Point_Array := Graham_Scan_Hull (Tri);
   begin
      ...
      Check (Same_Vertex_Set (H, G), "triangle matches Graham");
   ...
   declare
      ...
      GS : constant Point_Array := Graham_Scan_Hull (Sq);
      GR : constant Point_Array := Graham_Scan_Hull (Rect);
   begin
      ...
      Check (Same_Vertex_Set (HS, GS), "square matches Graham");
      ...
      Check (Same_Vertex_Set (HR, GR), "rect matches Graham");
   ...
   declare
      ...
      G : constant Point_Array := Graham_Scan_Hull (Cloud);
   begin
      ...
      Check (Same_Vertex_Set (H, G), "cloud matches Graham");
   ...
   declare
      ...
      G : constant Point_Array := Graham_Scan_Hull (Pent);
   begin
      ...
      Check (Same_Vertex_Set (H, G), "pentagon matches Graham");
   ...
   declare
      ...
      G : constant Point_Array := Graham_Scan_Hull (Oct);
   begin
      ...
      Check (Same_Vertex_Set (H, G), "octagon matches Graham");
   ...
   declare
      ...
      GH : constant Point_Array := Graham_Scan_Hull (Horz);
      GV : constant Point_Array := Graham_Scan_Hull (Vert);
      GD : constant Point_Array := Graham_Scan_Hull (Diag);
      GE : constant Point_Array := Graham_Scan_Hull (Edge);
   begin
      ...
      Check (Same_Vertex_Set (HH, GH), "horz matches Graham");
      ...
      Check (Same_Vertex_Set (HV, GV), "vert matches Graham");
      ...
      Check (Same_Vertex_Set (HD, GD), "diag matches Graham");
      ...
      Check (Same_Vertex_Set (HE, GE), "edge matches Graham");
   ...
   declare
      ...
      G : constant Point_Array := Graham_Scan_Hull (Dia);
   begin
      ...
      Check (Same_Vertex_Set (H, G), "diamond matches Graham");
      Check (Near (Signed_Area (H), R (2.0)), "diamond area 2");
   end;

   ------------------------------------------------------------------
   Section ("11. Random-ish classroom cloud vs oracles");
   ------------------------------------------------------------------
   declare
      Cloud : constant Point_Set :=
   ...
   begin
      ...
      Check (H'Length = G'Length, "cloud same count as Graham");
      Check (Same_Vertex_Set (H, A), "cloud vertex set = Andrew");
      Check (Same_Vertex_Set (H, G), "cloud vertex set = Graham");
   ...
   begin
      ...
      declare
         ...
         TG : constant Point_Array := Graham_Scan_Hull (Big);
      begin
         Check (T'Length = 16, "Max_Points ring → 16 hull");
         Check (Same_Vertex_Set (T, TA), "Max_Points matches Andrew");
         Check (Same_Vertex_Set (T, TG), "Max_Points matches Graham");
   ...
   ------------------------------------------------------------------
   Section ("14. Chan start is lowest-then-leftmost on hull");
   ------------------------------------------------------------------
   declare
      ...
      G : constant Point_Array := Graham_Scan_Hull (Pts);
   begin
      ...
      Check (Same_Vertex_Set (H, A), "pivot matches Andrew");
      Check (Same_Vertex_Set (H, G), "pivot matches Graham");
   end;

   ------------------------------------------------------------------
   Section ("15. Larger h forces squaring pass (many hull verts)");
   ------------------------------------------------------------------
   declare
      --  12-gon in convex position → h=12 > first guess H=4.
      Gon : Point_Array (1 .. 12);
      H, A, G : Point_Array (1 .. 12);
      HN : Point_Count;
   begin
      for I in Gon'Range loop
         declare
            Ang : constant Long_Float :=
              2.0 * Ada.Numerics.Pi * Long_Float (I - 1) / 12.0;
         begin
            Gon (I) :=
              P (Real (3.0 * Math.Cos (Ang)),
                 Real (3.0 * Math.Sin (Ang)));
         end;
      end loop;
      declare
         T : constant Point_Array := Convex_Hull (Gon);
         TA : constant Point_Array := Andrew_Monotone_Chain (Gon);
         TG : constant Point_Array := Graham_Scan_Hull (Gon);
      begin
         HN := T'Length;
         for I in T'Range loop
            H (I) := T (I);
         ...
         for I in TG'Range loop
            G (I) := TG (I);
         end loop;
         Check (HN = 12, "12-gon hull 12 (needs H≥12)");
         Check (Same_Vertex_Set (T, TA), "12-gon matches Andrew");
         Check (Same_Vertex_Set (T, TG), "12-gon matches Graham");
         Check (Is_CCW (T), "12-gon CCW");
         Check (Is_Strictly_Convex_CCW (T), "12-gon strictly convex");
      end;
      pragma Unreferenced (H, A, G);
   end;
